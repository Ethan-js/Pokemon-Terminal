...
def refreshpad(pad,y,x,window,Player,a=0,b=0):
    ...
    Player.to_b(int((screensize[0]-1)/2)+a,int((screensize[1]-1)/2)+b)
    Player.win.noutrefresh()
...
def get_move(pad,y,x,window,Player):
    a = 0
    b = 0
    while True:
        move = window.getch()
        if move == curses.KEY_DOWN:
            if a < 0:
                ...
                y += 1
            refreshpad(pad,y,x,window,Player,a,b)
...
class Sprite:
    def __init__(self,h,l,posy,posx,texts,window):
        ...
        if len(self.texts) > 1:
            ...
            self.win, self.panels[0] = make_panel(h,l,posy,posx,texts[0])
